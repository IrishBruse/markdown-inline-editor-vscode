...
Expected with `custom`: the table renders as a bordered grid with **word-wrapped cells**. Column width follows cell content up to a maximum (~400px); longer text wraps onto multiple lines inside the cell. Row heights grow with wrapped line count (the overlay may extend below the last source line when cells wrap heavily). Source GFM is hidden while the overlay is shown. Click inside the table to edit raw markdown (same as `selection-reveal`).
