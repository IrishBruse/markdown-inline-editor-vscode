...
Expected with `custom`: the table renders as a bordered grid with **word-wrapped cells** (one SVG band per source line). Column width follows cell content up to a maximum (~400px); longer text wraps within each line's band and may truncate when it would exceed one editor line of height. The overlay does not extend below the table's last source line, so the next section is not covered. Each band stays visible when its source line is in the viewport (scrolling within the table no longer hides the whole grid). Source GFM is hidden while overlays are shown. Click inside the table to edit raw markdown (same as `selection-reveal`).
