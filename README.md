...
| Tables | `tables.style` (`grid`), `tables.cjkWidthRatio` (`2`), `tables.maxColumnWidth` (`48`), `tables.rowSeparators` (`true`); set `tables.style` to `preview` for preview-pane layout (hidden pipes, fixed-width cells) |
